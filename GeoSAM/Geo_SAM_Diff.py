...
'''
去除了SAM中的提示编码器，添加了一个专门用于处理ndsm图像的子网络

训练过程中冻结了编码器的参数，仅对解码器、Geo-Adapter以及子网络进行训练

相较于Geo_SAM6,Geo_SAM13增加了中间特征解码的通道数
'''
...
class Ndsm_Encoder(nn.Module):
    def __init__(self):
        super(Ndsm_Encoder, self).__init__()
        self.mobileNet = mobilenet_v2(pretrained=True)

    def forward(self, ndsm):
        n1 = self.mobileNet.features[0:2](ndsm)  # [4, 16, 128, 128]
        n2 = self.mobileNet.features[2:4](n1)  # [4, 24, 64, 64]
        n3 = self.mobileNet.features[4:7](n2)  # [4, 32, 32, 32]
        n4 = self.mobileNet.features[7:17](n3)  # [4, 160, 16, 16]
        n5 = self.mobileNet.features[17:18](n4)  # [4, 320, 8, 8]
