...
class PromptMaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        src = image_embeddings + F.upsample(dense_prompt_embeddings, size=(16, 16), mode='bilinear', align_corners=False)# + prompt_token
...
if __name__ == '__main__':
    image_embeddings = torch.randn(2, 256, 16, 16).cuda()
    dense_prompt_embeddings = torch.randn(2, 256, 32, 32).cuda()
    image_pe = torch.randn(1, 256, 16, 16).cuda()